...
description: Design, install, and operate safe parallel AI-agent workflows across any Git repository. Use when the user wants multiple Codex, Claude, or other agents to work at the same time without overwriting each other; wants worktree/branch/task dispatch design; wants reusable agent prompts, path ownership, repo-local coordination files, preflight checks, or GitHub publishing guidance for multi-agent development.
---

# Multi-Agent Workflow

Use this skill to turn a repository into a safe workspace for parallel AI agents.
...
5. **Use generated prompts for cross-tool agents.**
   For Claude, Codex, or another tool, paste the generated prompt into that
   program. The prompt must include the exact worktree path, allowed paths,
   blocked paths, and final reporting requirements.
